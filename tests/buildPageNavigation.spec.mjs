...
test('given a build page path, normalizeBuildPageHref returns the canonical href', () => {
  ...
  assert.equal(normalizeBuildPageHref('https://example.test/app/deck-magic-3.html#top'), './deck-magic-3.html');
});
...
test('given a current deck magic page, rememberCurrentBuildPage stores both exact build page and deck magic page', () => {
  const storage = createMemoryStorage();
  const currentHref = rememberCurrentBuildPage(storage, { pathname: '/app/deck-magic-2.html' });

  assert.equal(currentHref, './deck-magic-2.html');
  assert.equal(getLastBuildPageHref(storage), './deck-magic-2.html');
  assert.equal(getLastDeckMagicPageHref(storage), './deck-magic-2.html');
});
...
test('given a build page href, getBuildPageTabId groups deck magic pages under one tab id', () => {
  ...
  assert.equal(getBuildPageTabId('./deck-magic-3.html'), 'deck-magic');
...
test('given saved deck magic history, getBuildPageSubnavEntries resolves the deck magic tab to the last deck magic page', () => {
  const storage = createMemoryStorage();
  storage.setItem('findit:last-deck-magic-page', './deck-magic-3.html');

  const entries = getBuildPageSubnavEntries(storage);

  assert.equal(entries.length, 3);
  assert.equal(entries[0].href, './deck-preview.html');
  assert.equal(entries[1].href, './deck-magic-3.html');
